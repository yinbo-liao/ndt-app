-- =============================================================================
-- NDT Management App — CONSOLIDATED SCHEMA (idempotent)
--
-- Combines migrations 001–005 into a single script safe to run via
-- Supabase SQL Editor. Uses IF NOT EXISTS / OR REPLACE / DO-block guards
...
-- ═══════════════════════════════════════════════════════════════
-- 2. TABLES (all 11 — FK ON DELETE rules applied at creation)
-- ═══════════════════════════════════════════════════════════════

-- 2.1 ndt_companies
CREATE TABLE IF NOT EXISTS ndt_companies (
...
CREATE INDEX IF NOT EXISTS idx_ndt_companies_active ON ndt_companies(active);
...
CREATE INDEX IF NOT EXISTS idx_users_company ON users(ndt_company_id);
...
CREATE INDEX IF NOT EXISTS idx_contractor_company ON ndt_contractor_register(ndt_company_id);
CREATE INDEX IF NOT EXISTS idx_contractor_status ON ndt_contractor_register(validation_status);
CREATE INDEX IF NOT EXISTS idx_contractor_month ON ndt_contractor_register(report_month);
CREATE INDEX IF NOT EXISTS idx_contractor_expire ON ndt_contractor_register(expire_date);
CREATE INDEX IF NOT EXISTS idx_contractor_deleted ON ndt_contractor_register(deleted_at) WHERE deleted_at IS NULL;
CREATE INDEX IF NOT EXISTS idx_contractor_created_by ON ndt_contractor_register(created_by);
...
CREATE INDEX IF NOT EXISTS idx_projects_qa ON projects(qa_incharge_id);
CREATE INDEX IF NOT EXISTS idx_projects_ndt_company ON projects(ndt_company_id);
...
CREATE INDEX IF NOT EXISTS idx_planning_project ON project_ndt_planning(project_id);
CREATE INDEX IF NOT EXISTS idx_planning_company ON project_ndt_planning(ndt_company_id);
CREATE INDEX IF NOT EXISTS idx_planning_status ON project_ndt_planning(testing_status);
CREATE INDEX IF NOT EXISTS idx_planning_dates ON project_ndt_planning(planned_start_date, planned_end_date);
CREATE INDEX IF NOT EXISTS idx_planning_discipline ON project_ndt_planning(discipline);
CREATE INDEX IF NOT EXISTS idx_planning_deploy_status ON project_ndt_planning(team_deploy_status);
CREATE INDEX IF NOT EXISTS idx_planning_accept_status ON project_ndt_planning(accept_status);
...
CREATE INDEX IF NOT EXISTS idx_deployments_planning ON ndt_team_deployments(project_ndt_planning_id);
CREATE INDEX IF NOT EXISTS idx_deployments_company ON ndt_team_deployments(ndt_company_id);
CREATE INDEX IF NOT EXISTS idx_deployments_shift ON ndt_team_deployments(shift);
CREATE INDEX IF NOT EXISTS idx_deployments_date ON ndt_team_deployments(deployment_date);
CREATE INDEX IF NOT EXISTS idx_deployments_status ON ndt_team_deployments(testing_status);
CREATE INDEX IF NOT EXISTS idx_deployments_supervisor ON ndt_team_deployments(ndt_supervisor_id);
CREATE INDEX IF NOT EXISTS idx_deployments_deleted ON ndt_team_deployments(deleted_at) WHERE deleted_at IS NULL;
CREATE INDEX IF NOT EXISTS idx_deployments_created_by ON ndt_team_deployments(created_by);
...
CREATE INDEX IF NOT EXISTS idx_assignments_user ON ndt_team_assignments(user_id);
CREATE INDEX IF NOT EXISTS idx_assignments_project ON ndt_team_assignments(project_id);
CREATE INDEX IF NOT EXISTS idx_assignments_company ON ndt_team_assignments(ndt_company_id);
CREATE INDEX IF NOT EXISTS idx_assignments_status ON ndt_team_assignments(status);
...
-- 2.10 ndt_professional_register
...
CREATE INDEX IF NOT EXISTS idx_professional_company ON ndt_professional_register(ndt_company_id);
CREATE INDEX IF NOT EXISTS idx_professional_status ON ndt_professional_register(certificate_status);
CREATE INDEX IF NOT EXISTS idx_professional_expire ON ndt_professional_register(expiry_date);
CREATE INDEX IF NOT EXISTS idx_professional_sector ON ndt_professional_register(working_sector);
CREATE INDEX IF NOT EXISTS idx_professional_deleted ON ndt_professional_register(deleted_at) WHERE deleted_at IS NULL;
CREATE INDEX IF NOT EXISTS idx_professional_created_by ON ndt_professional_register(created_by);

-- 2.11 ndt_professional_assignments
...
CREATE INDEX IF NOT EXISTS idx_prof_assign_planning ON ndt_professional_assignments(planning_id);
CREATE INDEX IF NOT EXISTS idx_prof_assign_professional ON ndt_professional_assignments(professional_id);
CREATE INDEX IF NOT EXISTS idx_prof_assign_status ON ndt_professional_assignments(status);
...
-- Apply updated_at trigger to all 9 tables that have the column
DO $$
DECLARE
    t TEXT;
BEGIN
    FOR t IN ARRAY[
        ...
        IF NOT EXISTS (
            ...
            WHERE event_object_table = t AND trigger_name = 'update_' || t || '_updated_at'
        ) THEN
            EXECUTE format(
...
-- 3.2 Audit log trigger function
...
-- Apply audit trigger to all 10 tables
DO $$
DECLARE
    t TEXT;
BEGIN
    FOR t IN ARRAY[
        'ndt_companies','users','ndt_contractor_register','projects',
        'project_ndt_planning','ndt_team_deployments','ndt_team_assignments',
        'audit_logs','ndt_professional_register','ndt_professional_assignments',
        'notifications'
    ] LOOP
        IF t = 'audit_logs' THEN CONTINUE; END IF; -- skip self-referencing
        IF NOT EXISTS (
            SELECT 1 FROM information_schema.triggers
            WHERE event_object_table = t AND trigger_name = 'audit_' || t
        ) THEN
            EXECUTE format(
...
-- 3.3 Certificate expiry notification trigger
...
BEGIN
    IF NOT EXISTS (
        SELECT 1 FROM information_schema.triggers
        WHERE event_object_table = 'ndt_contractor_register'
        AND trigger_name = 'trigger_certificate_expiry'
    ) THEN
...
-- 3.4 Auto-set validation_status to expired
...
BEGIN
    IF NOT EXISTS (
        SELECT 1 FROM information_schema.triggers
        WHERE event_object_table = 'ndt_contractor_register'
        AND trigger_name = 'check_cert_expiry_on_update'
    ) THEN
...
-- 3.5 Auto-set professional certificate_status to expired
...
BEGIN
    IF NOT EXISTS (
        SELECT 1 FROM information_schema.triggers
        WHERE event_object_table = 'ndt_professional_register'
        AND trigger_name = 'trigger_professional_cert_expiry'
    ) THEN
...
-- 3.6 Handle new user (auto-create profile on auth.users insert)
...
-- 4.1 Enable RLS on all 11 tables
DO $$
DECLARE
    t TEXT;
BEGIN
...
-- 4.2 Helper function: get user's company ID from JWT
...
-- 4.3 Admin policies (full access to all tables)
DO $$
DECLARE
    t TEXT;
BEGIN
    FOR t IN ARRAY[
        ...
        EXECUTE format(
            'DROP POLICY IF EXISTS "admin_all_%s" ON %I;
             CREATE POLICY "admin_all_%s" ON %I FOR ALL
...
-- 4.4 NDT Company policies

-- Company: read all projects
DROP POLICY IF EXISTS "company_read_projects" ON projects;
CREATE POLICY "company_read_projects" ON projects FOR SELECT
USING (auth.jwt()->>'role' = 'ndt_company');

-- Company: own contractor register
DROP POLICY IF EXISTS "company_own_contractor_register" ON ndt_contractor_register;
CREATE POLICY "company_own_contractor_register" ON ndt_contractor_register FOR ALL
USING (
...
-- Company: own planning
DROP POLICY IF EXISTS "company_own_planning" ON project_ndt_planning;
...
-- Company: own deployments
DROP POLICY IF EXISTS "company_own_deployments" ON ndt_team_deployments;
...
-- Company: own assignments
DROP POLICY IF EXISTS "company_own_assignments" ON ndt_team_assignments;
...
-- Company: view own profile
DROP POLICY IF EXISTS "company_view_own_profile" ON ndt_companies;
CREATE POLICY "company_view_own_profile" ON ndt_companies FOR SELECT
USING (
    auth.jwt()->>'role' = 'ndt_company'
    AND id = get_user_company_id()
);

-- Company: view own users
DROP POLICY IF EXISTS "company_view_own_users" ON users;
CREATE POLICY "company_view_own_users" ON users FOR SELECT
USING (
    auth.jwt()->>'role' = 'ndt_company'
    AND ndt_company_id = get_user_company_id()
);

-- Company: own professionals
DROP POLICY IF EXISTS "company_own_professionals" ON ndt_professional_register;
...
-- Company: own professional assignments (via planning chain)
DROP POLICY IF EXISTS "company_own_prof_assignments" ON ndt_professional_assignments;
CREATE POLICY "company_own_prof_assignments" ON ndt_professional_assignments FOR ALL
USING (
    auth.jwt()->>'role' = 'ndt_company'
    AND EXISTS (
...
-- 4.5 NDT Team policies

-- Team: view assigned projects
DROP POLICY IF EXISTS "team_view_assigned_projects" ON projects;
CREATE POLICY "team_view_assigned_projects" ON projects FOR SELECT
USING (
...
-- Team: view assigned planning
DROP POLICY IF EXISTS "team_view_assigned_planning" ON project_ndt_planning;
CREATE POLICY "team_view_assigned_planning" ON project_ndt_planning FOR SELECT
USING (
    auth.jwt()->>'role' = 'ndt_team'
    AND EXISTS (
        SELECT 1 FROM ndt_team_assignments a
...
-- Team: view own deployments
DROP POLICY IF EXISTS "team_view_own_deployments" ON ndt_team_deployments;
CREATE POLICY "team_view_own_deployments" ON ndt_team_deployments FOR SELECT
USING (
    auth.jwt()->>'role' = 'ndt_team'
    AND (
...
-- Team: update own deployments
DROP POLICY IF EXISTS "team_update_own_deployments" ON ndt_team_deployments;
CREATE POLICY "team_update_own_deployments" ON ndt_team_deployments FOR UPDATE
USING (
    auth.jwt()->>'role' = 'ndt_team'
    AND (
        ndt_supervisor_id = auth.uid()
...
-- Team: view own profile
DROP POLICY IF EXISTS "team_view_own_profile" ON users;
CREATE POLICY "team_view_own_profile" ON users FOR SELECT
USING (auth.jwt()->>'role' = 'ndt_team' AND id = auth.uid());

-- Team: view own company
DROP POLICY IF EXISTS "team_view_own_company" ON ndt_companies;
CREATE POLICY "team_view_own_company" ON ndt_companies FOR SELECT
USING (
    auth.jwt()->>'role' = 'ndt_team'
    AND id = get_user_company_id()
);

-- Team: view own assignments
DROP POLICY IF EXISTS "team_view_own_assignments" ON ndt_team_assignments;
CREATE POLICY "team_view_own_assignments" ON ndt_team_assignments FOR SELECT
USING (
    auth.jwt()->>'role' = 'ndt_team'
    AND user_id = auth.uid()
);

-- Team: view company professionals
DROP POLICY IF EXISTS "team_view_company_professionals" ON ndt_professional_register;
CREATE POLICY "team_view_company_professionals" ON ndt_professional_register FOR SELECT
USING (
    auth.jwt()->>'role' = 'ndt_team'
    AND ndt_company_id = get_user_company_id()
    AND deleted_at IS NULL
...
-- Team: view company professional assignments
DROP POLICY IF EXISTS "team_view_company_prof_assignments" ON ndt_professional_assignments;
CREATE POLICY "team_view_company_prof_assignments" ON ndt_professional_assignments FOR SELECT
USING (
    auth.jwt()->>'role' = 'ndt_team'
    AND EXISTS (
        SELECT 1 FROM project_ndt_planning p
...
-- 5.2 Daily Deployment Summary per Company
...
-- 5.6 Professional Register Summary
